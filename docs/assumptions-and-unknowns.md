...
| Exact Xray binding defaults | Listener exposure depends on defaults | Keep explicit `listen` values and preflight checks |
| Geosite/GeoIP version on user device | Routing can drift | Publish hashes and tested versions |
| Provider success rates by ISP/region | Provider routes can be region-specific | Keep provider status, issue templates, and lab evidence current |
...
| FakeDNS stale cache frequency | Recovery need varies | Keep recovery validation reports and lab evidence current |
