...
برنامه با صفحه **Dashboard** باز می‌شود. برای تازه‌واردها کارت **New here?** و صفحه **Getting Started** (قبلاً Tools) مسیر گام‌به‌گام را نشان می‌دهد. راهنمای انگلیسی کوتاه: `docs/getting-started.md`.

برای استفاده معمولی همین ترتیب کافی است:
...
مسیر راه‌اندازی مرحله‌ای و ابزارهای اختیاری را نگه می‌دارد. دکمه **Smart Tips** همان موتور `main.py advise` را به‌صورت محلی نشان می‌دهد. ابزارهای اضافی مثل نصب Playwright، نصب ابزار fingerprint، دانلود Xray Core و PyInstaller در بخش اختیاری مخفی شده‌اند تا صفحه اصلی شلوغ نشود.
