...
This repository keeps runtime files, documentation, and local diagnostics separate:

- The primary runtime config remains `Xray-config/MITM-DomainFronting.json`.
- Local certificates and keys such as `mycert.crt` and `mycert.key` must be generated by each user and are ignored by git.
...
The GUI opens on **Start Here** for new users. It shows the safe setup path first: check setup, generate a personal local CA, start the proxy, and run one page check. Advanced browser, profile, repair, and deeper project tools are hidden behind toggles so normal users do not have to sort through them.
...
On Windows, `py -3 main.py audit` is equivalent if `python` is not on `PATH`.

Expanded project checks:
