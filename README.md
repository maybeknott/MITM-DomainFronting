...
برنامه با صفحه **Start Here** باز می‌شود. مسیر پیشنهادی را به ترتیب انجام دهید:
...
صفحه **Run & Test** به شکل dashboard ساده‌تر طراحی شده است: وضعیت سیستم، Xray Core، پروکسی محلی، DNS و Network Mode در بالای صفحه دیده می‌شوند؛ سپس workflow، کنترل core، وضعیت browser proxy و quick actions نمایش داده می‌شود.
...
پنل سمت راست برنامه، Telemetry محلی را نشان می‌دهد: وضعیت زنده، سرعت دریافت و ارسال، مجموع ترافیک از زمان باز شدن برنامه، مدت اتصال، و تاریخچه فعالیت‌های خود برنامه. این داده‌ها از شمارنده‌های سیستم‌عامل و رویدادهای محلی GUI می‌آیند و محتوای ترافیک مرورگر را بررسی نمی‌کنند.
